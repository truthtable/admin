...
const DELETE_DELIVERY_BY_LIST_API = URL + "api/deleteDeliveryByIdList";
...
export const deleteDeliveryByIdList = (ids) => {
    return async (dispatch) => {
        try {
            ...
            dispatch(deleteDelivery());
            let response = await axios().post(DELETE_DELIVERY_BY_LIST_API, {
                ids: ids,
            });
            console.log("res", response.data);
            const deliveries = response.data;
            if (deliveries.success === false) {
                ...
                dispatch(deleteDeliverySuccess(ids));
            }
        } catch (error) {
            dispatch(fetchDeliveriesFailure(error.message));
        }

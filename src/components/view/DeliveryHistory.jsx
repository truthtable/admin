...
export default function DeliveryHistory() {
     ...
     const { columns, csvData, deliveriesMapList } = useMemo(() => {
          ...
          sortedDeliveries.forEach((delivery, i) => {
               ...
               if (urlParams.date_start && urlParams.date_end) {
                    const mDateStart = new Date(urlParams.date_start);
                    mDateStart.setHours(0, 0, 0, 0);
                    const mDateEnd = new Date(urlParams.date_end);
                    mDateEnd.setHours(23, 59, 59, 999);
                    const deliveryDate = new Date(delivery.created_at);

                    if (deliveryDate < mDateStart || deliveryDate > mDateEnd) return;
               }
          ...
          >
               ...
               >
                    ...
                    <Autocomplete
                         ...
                         value={[{ id: null, label: "Show All" }, ...CUSTOMER_LIST].find(option => option.id === customerId) || null}
